import {API, Building, BUILDING_API, BuildingStoreModel, Config, Enumerated, session, UUID} from "@zeitwert/ui-model";
...
export default class StratusImport {
	...
	private importBuilding = async (stratusBuilding: StratusBuilding, onProgress: (progress: number) => void) => {
		...
		try {
			...
			onProgress(2);
			store.building!.currentRating!.setField("ratingDate", new Date().setFullYear(stratusBuilding.conditionYear!));
			await store.building!.currentRating!.setPartCatalog({id: "C99", name: "Stratus"}/*stratusBuilding.partCatalog*/);
			onProgress(3);
			const changes = Object.assign({}, getSnapshot(store.building!) as any);
			changes.currentRating = Object.assign({}, changes.currentRating);
			changes.currentRating.elements = [];
			stratusBuilding.elements.forEach((element) => {
				...
				changes.currentRating.elements.push(buildingElement);
			});
