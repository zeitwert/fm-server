...
abstract class JooqAggregatePersistenceProviderBase<A : Aggregate> : AggregatePersistenceProvider<A> {
    ...
    protected open fun toAggregate(record: UpdatableRecord<*>, aggregate: A) {
        ...
        (aggregate.getProperty("createdByUser") as? ReferenceProperty<ObjUser>)?.id = getRecordField(record, "createdByUserId")
        (aggregate.getProperty("createdAt") as? BaseProperty<OffsetDateTime?>)?.value = getRecordField(record, "createdAt") as? OffsetDateTime
        (aggregate.getProperty("modifiedByUser") as? ReferenceProperty<ObjUser>)?.id = getRecordField(record, "modifiedByUserId")
        (aggregate.getProperty("modifiedAt") as? BaseProperty<OffsetDateTime?>)?.value = getRecordField(record, "modifiedAt") as? OffsetDateTime
    }
