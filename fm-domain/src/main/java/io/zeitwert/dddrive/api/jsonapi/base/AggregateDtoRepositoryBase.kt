...
import io.zeitwert.api.jsonapi.ResourceRepository
import io.zeitwert.app.model.SessionContext
import io.zeitwert.fm.oe.model.ObjUser
import io.zeitwert.persist.sql.util.QuerySpecConverter
 ...
 */
abstract class AggregateDtoRepositoryBase<A : Aggregate, R : ResourceDto>(
	...
	companion object {
		...
		private val logger = LoggerFactory.getLogger(AggregateDtoRepositoryBase::class.java)
	}
	...
	override fun <S : R> save(dto: S): S {
		...
		try {
			toAggregate(dto, aggregate)
			if (dto.hasOperation(ResourceRepository.CalculationOnlyOperation)) {
