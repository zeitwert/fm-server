...
open class ObjContactPersistenceProvider : JooqObjPersistenceProviderBase<ObjContact>() {
    ...
    override fun fromAggregate(aggregate: ObjContact): UpdatableRecord<*> {
    ...
    override fun storeExtension(aggregate: ObjContact) {
        ...
        record.contactRoleId = (aggregate.getProperty("contactRole") as? EnumProperty<CodeContactRole>)?.value?.id
        record.salutationId = (aggregate.getProperty("salutation") as? EnumProperty<CodeSalutation>)?.value?.id
    ...
    @Suppress("UNCHECKED_CAST")
    private fun storeAddressParts(aggregate: ObjContact, objId: Int) {
        val addressList = (aggregate.getProperty("addressList") as? PartListProperty<ObjContactPartAddress>)?.parts ?: return

        // Delete existing addresses
        dslContext().deleteFrom(Tables.OBJ_CONTACT_PART_ADDRESS)
        ...
        addressList.forEachIndexed { index, address ->
            ...
            record.addressChannelId = (address.getProperty("addressChannel") as? EnumProperty<CodeAddressChannel>)?.value?.id
    ...
    @Suppress("UNCHECKED_CAST")
    override fun loadExtension(aggregate: ObjContact, objId: Int?) {
        ...
        record.titleId?.let { id ->
            (aggregate.getProperty("title") as? EnumProperty<CodeTitle>)?.value =
                CodeTitle.getTitle(id)
        }
    ...
    @Suppress("UNCHECKED_CAST")
    private fun loadAddressParts(aggregate: ObjContact, objId: Int) {
        val addressList = aggregate.getProperty("addressList") as? PartListProperty<ObjContactPartAddress> ?: return
