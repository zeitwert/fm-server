...
abstract class ObjContactBase(
	...
	override fun getRepository(): ObjContactRepository = super.getRepository() as ObjContactRepository
	...
	override fun getAccount(): ObjAccount? = _account.value

	// Address list implementation

	override fun getMailAddressList(): List<ObjContactPartAddress> = _addressList.parts.filter { it.isMailAddress == true }

	override fun getMailAddress(addressId: Int?): java.util.Optional<ObjContactPartAddress> = java.util.Optional.ofNullable(_addressList.parts.find { it.id == addressId && it.isMailAddress == true })

	override fun clearMailAddressList() {
		getMailAddressList().forEach { _addressList.removePart(it.id) }
	}

	override fun addMailAddress(): ObjContactPartAddress = _addressList.addPart(null)

	override fun removeMailAddress(addressId: Int?) {
		_addressList.removePart(addressId)
	}

	override fun getElectronicAddressList(): List<ObjContactPartAddress> = _addressList.parts.filter { it.isMailAddress == false }

	override fun getElectronicAddress(addressId: Int?): java.util.Optional<ObjContactPartAddress> = java.util.Optional.ofNullable(_addressList.parts.find { it.id == addressId && it.isMailAddress == false })

	override fun clearElectronicAddressList() {
		getElectronicAddressList().forEach { _addressList.removePart(it.id) }
	}

	override fun addElectronicAddress(): ObjContactPartAddress = _addressList.addPart(null)
