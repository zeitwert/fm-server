...
enum class CodeBuildingPart(
    ...
    fun getNextRestoration(elementValue: Double, ratingYear: Int, condition: Double): ProjectionPeriod {
        var startYear = 0
        var restorationYear = 0
        var restorationCosts: Double
        ...
        return ProjectionPeriod.builder()
            ...
            .restorationCosts(round(restorationCosts * elementValue))
            .build()
    }
    ...
    fun getTimeValue(relativeAge: Double): Double {
        ...
        return c0 +
            c1 * relativeAge +
            c2 * relativeAge.pow(2) +
            c3 * relativeAge.pow(3) +
            c4 * relativeAge.pow(4) +
            c5 * relativeAge.pow(5) +
            c6 * relativeAge.pow(6) +
            c7 * relativeAge.pow(7) +
            c8 * relativeAge.pow(8) +
            c9 * relativeAge.pow(9) +
            c10 * relativeAge.pow(10)
    }
    ...
    private fun fDerivative(relativeAge: Double): Double {
        return c1 +
            2 * c2 * relativeAge +
            3 * c3 * relativeAge.pow(2) +
            4 * c4 * relativeAge.pow(3) +
            5 * c5 * relativeAge.pow(4) +
            6 * c6 * relativeAge.pow(5) +
            7 * c7 * relativeAge.pow(6) +
            8 * c8 * relativeAge.pow(7) +
            9 * c9 * relativeAge.pow(8) +
            10 * c10 * relativeAge.pow(9)
    }
    ...
    fun getProjection(
        ...
        require(condition in 0.0..1.0) { "valid condition (0 <=$condition <= 1)" }
        require(duration <= 100) { "duration <= 100" }

        val maxProjectionYear = startYear + minOf(100, duration)
        ...
        val periodList = mutableListOf<ProjectionPeriod>()
        ...
        for (simYear in ratingYear..maxProjectionYear) {
            ...
            if (simYear >= startYear) {
                val period = ProjectionPeriod.builder()
                    .year(simYear)
                    .originalValue(elementValue)
    ...
    fun getTechRate(): Double {
        return when {
            id in FULL_TECH_RATES -> 1.0
            id in HALF_TECH_RATES -> 0.5
            else -> 0.0
        }
    }

    companion object Enumeration : EnumerationBase<CodeBuildingPart>(CodeBuildingPart::class.java) {
        private val FULL_TECH_RATES = listOf("P6", "P7", "P8", "P54", "P55", "P9", "P50", "P51", "P10", "P63", "P64", "P65", "P66")
        private val HALF_TECH_RATES = listOf("P12", "P60", "P61", "P62")
        ...
        @JvmStatic
        fun getTechRate(techPart: Double): Double {
        ...
        fun getMaintenanceRate(timeValue: Double): Double {
            return when {
                timeValue >= 1.0 -> 0.5
                timeValue >= 0.94 -> getRatio(timeValue, 0.94, 1.0, 0.64, 0.5)
                timeValue >= 0.85 -> getRatio(timeValue, 0.85, 0.94, 1.1, 0.64)
                timeValue >= 0.75 -> getRatio(timeValue, 0.75, 0.85, 2.0, 1.1)
                timeValue >= 0.67 -> getRatio(timeValue, 0.67, 0.75, 2.0, 2.0)
                timeValue >= 0.60 -> getRatio(timeValue, 0.60, 0.67, 0.5, 2.0)
                else -> getRatio(timeValue, 0.00, 0.60, 0.75, 0.5)
            }
        }

        private fun getRatio(value: Double, lowBound: Double, highBound: Double, lowValue: Double, highValue: Double): Double {
            return when {
                value <= lowBound -> lowValue
                value >= highBound -> highValue
                else -> lowValue + (value - lowBound) / (highBound - lowBound) * (highValue - lowValue)
            }
        }
    }
}
