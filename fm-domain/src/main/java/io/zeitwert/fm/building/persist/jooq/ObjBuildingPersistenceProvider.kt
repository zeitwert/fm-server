...
open class ObjBuildingPersistenceProvider : JooqObjPersistenceProviderBase<ObjBuilding>() {
	...
	override fun storeExtension(aggregate: ObjBuilding) {
		...
		record.historicPreservationId =
			(aggregate.getProperty("historicPreservation") as? EnumProperty<CodeHistoricPreservation>)?.value?.id
		...
		record.buildingSubTypeId =
			(aggregate.getProperty("buildingSubType") as? EnumProperty<CodeBuildingSubType>)?.value?.id
