...
class DocumentGenerationServiceImpl : DocumentGenerationService {
  ...
  @Autowired lateinit var requestCtx: RequestContext

  @Autowired lateinit var evaluationService: BuildingEvaluationService

  @Value("classpath:templates/Building Evaluation Template.docx")
  lateinit var templateFile: Resource

  @Value("classpath:templates/missing.jpg") lateinit var missingImage: Resource

  @Autowired lateinit var asposeConfig: AsposeConfig
  ...
  private fun insertCoverFoto(doc: Document?, building: ObjBuilding) {
  ...
  @Throws(IOException::class)
  private fun insertLocationImage(doc: Document?, building: ObjBuilding) {
    val imageContent: ByteArray?
  ...
  private fun fillOptRenovationTable(doc: Document, evaluationResult: BuildingEvaluationResult) {
    ...
    optRenovationTable.firstRow.nextSibling.nextSibling.remove()
  }
  ...
  private fun fillCostsChart(doc: Document, evaluationResult: BuildingEvaluationResult) {
    ...
    valueChart.series.get(0).marker.symbol = MarkerSymbol.CIRCLE
    valueChart.series.get(0).marker.size = 5

    valueChart.series.get(1).marker.symbol = MarkerSymbol.CIRCLE
    valueChart.series.get(1).marker.size = 5
  ...
  private fun fillCostsTable(doc: Document, evaluationResult: BuildingEvaluationResult) {
  ...
  private fun fillOnePagerCostsChart(doc: Document, evaluationResult: BuildingEvaluationResult) {
    val periodCount = evaluationResult.periods!!.count { p: EvaluationPeriod? -> p?.year != null }
    val years = arrayOfNulls<String>(periodCount)
    val maintenanceCosts = DoubleArray(periodCount)
    val restorationCosts = DoubleArray(periodCount)
  ...
  private fun fillOnePager(doc: Document, evaluationResult: BuildingEvaluationResult) {
    ...
    onePagerDetailsTable.firstRow.nextSibling.nextSibling.remove()
  ...
  private fun getRatingLineVOffset(lineNr: Int): Double {
  ...
  private fun getNthNextSibling(cell: Node, n: Int): Cell {
    if (n >= 0) {
