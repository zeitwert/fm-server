...
open class DocTestPersistenceProvider : JooqDocPersistenceProviderBase<DocTest>() {
    ...
    override fun getRepository(): DocTestRepository = _repository
    ...
    override fun fromAggregate(aggregate: DocTest): UpdatableRecord<*> {
    ...
    override fun storeExtension(aggregate: DocTest) {
        ...
        if (existingRecord != null) {
            ...
            dslContext().update(DSL.table("doc_test"))
                ...
                .set(DSL.field("ref_obj_id", Int::class.java), (aggregate.getProperty("refObj") as? ReferenceProperty<ObjTest>)?.id as? Int)
                .set(DSL.field("ref_doc_id", Int::class.java), (aggregate.getProperty("refDoc") as? ReferenceProperty<DocTest>)?.id as? Int)
                .where(DSL.field("doc_id", Int::class.java).eq(docId))
                .execute()
        } else {
            // Insert new record
            dslContext().insertInto(DSL.table("doc_test"))
                .columns(
    ...
    @Suppress("UNCHECKED_CAST")
    override fun loadExtension(aggregate: DocTest, docId: Int?) {
