...
open class ObjTestPersistenceProvider : JooqObjPersistenceProviderBase<ObjTest>() {
    ...
    private lateinit var _repository: ObjTestRepository
    ...
    @Autowired
    @Lazy
    fun setRepository(repository: ObjTestRepository) {
        this._repository = repository
    }

    override fun dslContext(): DSLContext = _dslContext

    override fun getRepository(): ObjTestRepository = _repository
    ...
    override fun fromAggregate(aggregate: ObjTest): UpdatableRecord<*> {
    ...
    override fun storeExtension(aggregate: ObjTest) {
        ...
        val existingRecord = dslContext().fetchOne(
            DSL.table("obj_test"),
            DSL.field("obj_id", Int::class.java).eq(objId)
        )

        if (existingRecord != null) {
            // Update existing record
            dslContext().update(DSL.table("obj_test"))
                .set(DSL.field("tenant_id", Int::class.java), aggregate.tenantId as? Int)
                .set(DSL.field("account_id", Int::class.java), (aggregate.getProperty("accountId") as? BaseProperty<Int?>)?.value)
                .set(DSL.field("short_text", String::class.java), (aggregate.getProperty("shortText") as? BaseProperty<String?>)?.value)
                .set(DSL.field("long_text", String::class.java), (aggregate.getProperty("longText") as? BaseProperty<String?>)?.value)
                .set(DSL.field("date", LocalDate::class.java), (aggregate.getProperty("date") as? BaseProperty<LocalDate?>)?.value)
                .set(DSL.field("int", Int::class.java), (aggregate.getProperty("int") as? BaseProperty<Int?>)?.value)
                .set(DSL.field("is_done", Boolean::class.java), (aggregate.getProperty("isDone") as? BaseProperty<Boolean?>)?.value)
                .set(DSL.field("json", JSON::class.java), (aggregate.getProperty("json") as? BaseProperty<JSON?>)?.value)
                .set(DSL.field("nr", BigDecimal::class.java), (aggregate.getProperty("nr") as? BaseProperty<BigDecimal?>)?.value)
                .set(DSL.field("test_type_id", String::class.java), (aggregate.getProperty("testType") as? EnumProperty<CodeTestType>)?.value?.id)
                .set(DSL.field("ref_test_id", Int::class.java), (aggregate.getProperty("refTest") as? ReferenceProperty<ObjTest>)?.id as? Int)
                .where(DSL.field("obj_id", Int::class.java).eq(objId))
                .execute()
        } else {
            // Insert new record
            dslContext().insertInto(DSL.table("obj_test"))
                .columns(
    ...
    @Suppress("UNCHECKED_CAST")
    override fun loadExtension(aggregate: ObjTest, objId: Int?) {
        ...
        (aggregate.getProperty("refTest") as? ReferenceProperty<ObjTest>)?.id = record.get("ref_test_id", Int::class.java)
