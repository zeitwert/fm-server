...
create or replace view migr_obj_contact_part_anniversary_v
as
select  id,
        obj_id,
				key,
				...
				anniversary_notification_id,
				anniversary_template_id
from    obj_contact_part_anniversary;


create or replace function insert_migr_obj_contact_part_anniversary_v()
returns trigger
as
$func$
declare
...
begin
	select obj_id into new_id from obj_contact where intl_key = new.key;
	insert into obj_contact_part_anniversary(
		id,
		obj_id,
		key,
		anniversary_type_id,
		...
		new_id,
		new.key,
		new.anniversary_type_id,
		--
		to_date(new.start_date, 'dd.mm.yyyy'),
		new.anniversary_notification_id,
		new.anniversary_template_id
	);
	return new;
end
$func$
...
create trigger migr_obj_contact_part_anniversary_v$ins
instead of insert on migr_obj_contact_part_anniversary_v
for each row execute procedure insert_migr_obj_contact_part_anniversary_v();


create trigger migr_obj_contact_v$ins
instead of insert on migr_obj_contact_v
for each row execute procedure insert_migr_obj_contact_v();
