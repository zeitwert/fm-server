...
abstract class AggregateRepositoryBase<A : Aggregate>(
...
) : AggregateRepository<A>, AggregateRepositorySPI<A> {

  private val objCache = Caffeine.newBuilder().maximumSize(200).recordStats().build<Any, A>()
  ...
  override fun idToString(id: Any?): String? =
          if (id == null) null else this.persistenceProvider.idToString(id)

  override fun idFromString(id: String?): Any? =
          if (id == null) null else this.persistenceProvider.idFromString(id)
  ...
  override fun get(id: Any): A =
          this.objCache.get(id, Function { aggrId: Any? -> this.get(aggrId!!, true) })
